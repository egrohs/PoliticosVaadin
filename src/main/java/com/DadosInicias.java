...
public class DadosInicias {
	...
	final static String[] groupsNames = { "Corporate Development", "Human Resources", "Legal", "Environment",
			"Quality Assurance", "Research and Development", "Production", "Sales", "Marketing" };
	final static String[] officeNames = { "London", "New York", "Tokyo", "Turku" };
	final static String[] fnames = { "Peter", "Alice", "Joshua", "Mike", "Olivia", "Nina", "Alex", "Rita", "Dan",
			"Umberto", "Henrik", "Rene", "Lisa", "Marge" };
	final static String[] lnames = { "Smith", "Gordon", "Simpson", "Brown", "Clavel", "Simons", "Verne", "Scott",
	...
	public static void create() {
		EntityManager em = Persistence.createEntityManagerFactory("politicos").createEntityManager();
		em.getTransaction().begin();
		Random r = new Random(0);
		/*
		 * for (String o : officeNames) { Partido geoGroup = new Partido();
		 * geoGroup.setName(o); for (String g : groupsNames) { Partido group =
		 * new Partido(); group.setName(g); Set<Politico> gPersons = new
		 * HashSet<Politico>();
		 * 
		 * int amount = r.nextInt(15) + 1; for (int i = 0; i < amount; i++) {
		 * Politico p = new Politico();
		 * p.setFirstName(fnames[r.nextInt(fnames.length)]);
		 * p.setLastName(lnames[r.nextInt(lnames.length)]);
		 * p.setCity(cities[r.nextInt(cities.length)]); p.setPhoneNumber(
		 * "+358 02 555 " + r.nextInt(10) + r.nextInt(10) + r.nextInt(10) +
		 * r.nextInt(10)); int n = r.nextInt(100000); if (n < 10000) { n +=
		 * 10000; } p.setZipCode("" + n);
		 * p.setStreet(streets[r.nextInt(streets.length)]);
		 * p.setPartidos(group); gPersons.add(p); em.persist(p); }
		 * group.setParent(geoGroup); group.setPersons(gPersons);
		 * em.persist(group); } em.persist(geoGroup); }
		 */
